...
@Controller
public class TranslatingOrdListController {
private static final Logger logger = Logger.getLogger(TranslatingOrdListController.class);
	
	@RequestMapping("/toTranslateOrderList")
     ...
     * 订单信息查询
     */
    @RequestMapping("/getTranslatingOrdData")
    @ResponseBody
     ...
     * 订单信息导出
     */
    @RequestMapping("/translatingExport")
    @ResponseBody
    public void  export(HttpServletRequest request, HttpServletResponse response, OrderPageQueryParams queryRequest) {
	  ...
	  //获取配置中的导出最大数值
		...
		if(!CollectionUtil.isEmpty(orderList)){
            ...
            String[] titles = new String[]{"订单来源", "订单类型", "订单编号", "下单时间", "昵称", "语种方向","订单级别","订单金额","实付金额","领取时间","交稿剩余时间","订单状态"};
    		String[] fieldNames = new String[]{"chlId", "orderType", "orderId", "orderTime",
    				"userName", "langire","orderLevel","totalFee","realFee","lockTime","remaningTime","state"};
