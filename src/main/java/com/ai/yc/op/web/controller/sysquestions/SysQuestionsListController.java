...
public class SysQuestionsListController {
	...
	@ResponseBody
	public ResponseData<PageInfo<QuestionsPageVo>> getList(HttpServletRequest request, String bid, String questionType)
			throws Exception {
		...
		try {
			...
			QuestionsPageQueryResponse queryQuestionsPageQueryResponse = querySysQuestionsSV
					.queryQuestionsPage(questionsPageQueryRequest);
			...
			if (queryQuestionsPageQueryResponse.getResponseHeader().isSuccess()) {
				...
				responseData = new ResponseData<PageInfo<QuestionsPageVo>>(ResponseData.AJAX_STATUS_SUCCESS, "查询成功",
						resultPageInfo);
			} else {
				responseData = new ResponseData<PageInfo<QuestionsPageVo>>(ResponseData.AJAX_STATUS_FAILURE, "查询失败",
						null);
			}
		} catch (Exception e) {
			logger.error("查询题目列表失败：", e);
			responseData = new ResponseData<PageInfo<QuestionsPageVo>>(ResponseData.AJAX_STATUS_FAILURE, "查询信息异常",
					null);
		}
		return responseData;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/uploadChoice")
	@ResponseBody
	public ResponseData<Boolean> uploadChoice(HttpServletRequest request, String bid,String questionType) throws Exception {
		// 判断请求是否为文件上传请求，若不是，则直接返回错误信息
		  if(request instanceof MultipartHttpServletRequest){
				...
				if(questionType.equals("0")){
					List<SaveSysQuestions> infos = ImportEmployeeChoice.importEmployeeByPoi(mFile.getInputStream());
					// 遍历解析Excel的实体集合
					for (SaveSysQuestions req : infos) {
						IQuerySysQuestionsSV querySysQuestionsSV = DubboConsumerFactory.getService(IQuerySysQuestionsSV.class);
						req.setBid(bid);
						BaseResponse saveSysQuestions = querySysQuestionsSV.saveSysQuestions(req);
						if (saveSysQuestions.getResponseHeader().getIsSuccess() == true) {
							return new ResponseData<Boolean>(ResponseData.AJAX_STATUS_SUCCESS, "导入选择题目成功", true);
						}
					}
				}else{
					List<SaveSysQuestions> infos = ImportEmployee.importEmployeeByPoi(mFile.getInputStream());
					// 遍历解析Excel的实体集合
					for (SaveSysQuestions req : infos) {
						IQuerySysQuestionsSV querySysQuestionsSV = DubboConsumerFactory.getService(IQuerySysQuestionsSV.class);
						req.setBid(bid);
