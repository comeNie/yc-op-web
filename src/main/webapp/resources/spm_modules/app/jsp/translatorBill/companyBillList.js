define('app/jsp/translatorBill/companyBillList', function (require, exports, module) {
    'use strict';
    var $=require('jquery'),
    ...
    var OrderListPager = Widget.extend({
    		...
    		$.ajax({
				...
				success : function(data) {
					if(data.statusCode==1){
						//跳到列表页面
						window.location.href=_base+"/companyBill/toCompanyBillList?random="+Math.random();
					}else{
						var d = Dialog({
							title: '消息',
							content:"修改失败",
    	...
    	_detailPage:function(billId){
			window.location.href = _base+"/companyBill/toCompanyDetailBillList?billID="+billId;
		}
    });
